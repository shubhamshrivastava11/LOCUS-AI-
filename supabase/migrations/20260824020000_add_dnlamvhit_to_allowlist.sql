...
--
-- Re-adds dnlamvhit@gmail.com to the allowlist. Their old tenant + login
-- were removed in the earlier account cleanup; their auth.users row no
-- longer exists, so unlike the Olubusola case this is a clean re-add - the
-- next sign-in will insert a fresh auth.users row and handle_new_user()
-- will fire normally, no manual tenant creation needed.
